...
def getD(shape, center, r, sliceThickness=None, method="exp", dtype="float64", ):
    ...
    print("Dmax, Mean, Median", np.max(D), np.mean(D), np.median(D))
    return D
...
def getMsDoubleExp(D, r, prescription=1, A1=65.7, a1=0.094, A2=50.8, a2=0.006):
    ...
    print("Dosemax, Mean, Median", np.max(M), np.mean(M), np.median(M))
    return M
...
def resampleScalarVolumeBrains(inputVolumeNode, referenceVolumeNode, outputName):
    # Set parameters
    parameters = {}
    parameters["inputVolume"] = inputVolumeNode
    try:
        outputModelNode = slicer.mrmlScene.AddNewNodeByClass('vtkMRMLScalarVolumeNode', outputName)
    except:
...
def createVolumeNode(doseVolume, referenceNode, volumeName):
    ...
    displayNode.SetAndObserveColorNodeID('vtkMRMLColorTableNodeIsodose_ColorTable_Relative')
